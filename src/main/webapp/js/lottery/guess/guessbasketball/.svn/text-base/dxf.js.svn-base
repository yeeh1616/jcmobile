...
$(document).ready(function () {
	...
	// 单击事件
	$(".bigsmall_b, .bigsmall_s").click(function(){
			...
			if ( $(this).attr("href") == "#out_tickets" ) {
				...
				$("#betUlList").find("li").each(function(index){
					if ($(this).find(".active").length > 0) {
						...
						var sty = ["part2", "part4"];
						$(this).find(".bigsmall_b,.bigsmall_s").each(function(index){
	...
	for ( var i = 0; i < schedules.length; i++ ) {
		...
		if ( date == obj.id.substring(0,8) ) {
			...
			html += '<div class="part_r">'+
						...
						'</div>'+
						'<div class="bottom">'+
						'<div class="bigsmall_b" bet="0" betName="大分">'+
						'<a href="#">大分'+odds[0]+'</a>'+
						'</div>'+
						'<div class="bigsmall_s " bet="1" betName="小分">'+
						'<a href="#">小分'+odds[1]+'</a>'+
						'</div>'+
						'</div>'+
					'</div>';
