			...
			if ( $(this).attr("href") == "#out_tickets" ) {
				...
				$("#betUlList").find("li").each(function(index){
					if ($(this).find(".active").length > 0) {
						...
						var hag = [$(this).attr("host"), $(this).attr("guest")];
						var sty = ["part2", "part4"];
	...
	for ( var i = 0; i < schedules.length; i++ ) {
		...
		if ( date == obj.id.substring(0,8) ) {
			html += '<li id="'+obj.id+'" host="'+obj.hostname+'" guest="'+obj.guestname+'">'+
						...
						'</div>';
						if ( obj.ggrq == "-1" ) {
							html += '<div class="part3 s " bet="1-3" betName="主胜">'+
							'<a href="#"><div style=" border-bottom:none">'+obj.hostname+'</div>'+
							'<div style=" border-top:none">主胜<span style=" margin-left:5px;">'+spfOdds[0]+'</span>'+
							'</div></a></div>'+
							'<div class="part3 f " bet="2-0" betName="客不败">'+
							'<a href="#"><div style=" border-bottom:none">'+obj.guestname+'</div>'+
							'<div style=" border-top:none">客不败<span style=" margin-left:5px;">'+rqSpfOdds[2]+'</span>'+
							'</div></a></div>';
						} else {
							html += '<div class="part3 s " bet="2-3" betName="主不败">'+
							'<a href="#"><div style=" border-bottom:none">'+obj.hostname+'</div>'+
							'<div style=" border-top:none">主不败<span style=" margin-left:5px;">'+rqSpfOdds[0]+'</span>'+
							'</div></a></div>'+
							'<div class="part3 f " bet="1-0" betName="客胜">'+
							'<a href="#"><div style=" border-bottom:none">'+obj.guestname+'</div>'+
							'<div style=" border-top:none">客胜<span style=" margin-left:5px;">'+spfOdds[2]+'</span>'+
							'</div></a></div>';
						}
