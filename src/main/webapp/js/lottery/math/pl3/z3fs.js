...
$(document).ready(function () {
	...
	var html = '';
	for ( var i = 0; i < 10; i++ ) {
		html += '<li v='+i+'><a href="#">'+i+'</a><p>'+z3FsOmit[0].dmos[i].omit+'</p> </li>';
	}
	...
	$("#qrBtnId").click(function(){
		...
		if ( counter < 2 ) {
			...
			// 填充投注数据
			var betNum = combAlg.calcCombNum(counter,2)*2;
			var t = "", code = "";
	...
	$(".cp_button .jx").click(function(){
		var code = "", t = "";
		var array = rarray(2, 10);
		for ( var i = 0; i < 2; i++ ) {
			code += array[i] + ",";
			t += '<span>'+array[i]+"</span>";
		}
		code = code.substring(0, code.length-1);
		var html = '<li id="li_1" betNum="2" code="'+code+'">'+
					'<a class="ui-link" href="#"><img width="53" height="53" src="/mobile/images/guanbi.png" onclick="removeT(this)"></a>'+
					'<div class="zx_no">';
			html += t;
			html +=	'<div class="clear"></div>'+
					'</div>'+
					'<div class="zhushu">复式 2注4元</div>'+
					'</li>';
		$(html).appendTo($("#ul_list"));
		calcTotal();
	});
...
function calc() {
	...
	var betNum = 0;
	if ( counter >= 2 ) {
		betNum = combAlg.calcCombNum(counter,2)*2;
	}
