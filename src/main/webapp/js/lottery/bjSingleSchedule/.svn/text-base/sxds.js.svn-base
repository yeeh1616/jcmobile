...
JCZQ_INDEX_BF.put("0", "上单");
JCZQ_INDEX_BF.put("1", "上双");
JCZQ_INDEX_BF.put("2", "下单");
JCZQ_INDEX_BF.put("3", "下双");
var dateUtils = new DateUtils();
var guess = new SingleSchedule(6);
			...
			if ( $(this).attr("href") == "#out_tickets" ) {
				...
				$("#betUlList").find("li").each(function(index){
					if ($(this).find(".active").length > 0) {
						...
						$(this).find(".active").each(function(index){
							html += '<span class="active" bet="'+$(this).attr("bet")+'">'+JCZQ_INDEX_BF.get($(this).attr("bet"))+'</span>';
						});
						html	+=		'</div>'+
