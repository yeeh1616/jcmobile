...
$(document).ready(function () {
	...
	$(".qd-button").click(function(){
		...
		if ( len < 3 ) {
			tigMessage_foundation("至少选择3场比赛！");
	...
	for ( var i = 0; i < schedules.length; i++ ) {
		...
		if ( date == obj.uuid.substring(0,8) ) {
			html += '<li id="'+obj.uuid+'" host="'+obj.hostTeam+'" guest="'+obj.guestTeam+'">'+
						...
						'</div>';
						html += '<div class="part3 s " bet="3" betName="主胜">'+
						'<a href="#"><div style=" border-bottom:none">'+obj.hostTeam+'<span class="red">'+obj.reserved1+'</span></div>'+
						'<div style=" border-top:none">主胜<span style=" margin-left:5px;">'+odds[0]+'</span>'+
						'</div></a></div>'+
						'<div class="part3 f " bet="0" betName="主败">'+
						'<a href="#"><div style=" border-bottom:none">'+obj.guestTeam+'</div>'+
						'<div style=" border-top:none">主败<span style=" margin-left:5px;">'+odds[1]+'</span>'+
						'</div></a></div>';
